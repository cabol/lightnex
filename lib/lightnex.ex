defmodule Lightnex do
  ...
  * Block height and version
  ...
  @spec connect_peer(Conn.t(), String.t(), String.t(), keyword()) ::
          {:ok, Lightning.ConnectPeerResponse.t()} | {:error, term()}
  def connect_peer(conn, pubkey, host, opts \\ [])
  ...
  def connect_peer(%Conn{} = conn, pubkey, host, opts)
    ...
    case Lightning.Stub.connect_peer(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        Logger.info("Connected to peer: #{pubkey}@#{host}")
        {:ok, response}

      {:error, %GRPC.RPCError{message: msg}} = error ->
        # Already connected is not really an error
        if String.contains?(msg, "already connected") do
          Logger.debug("Peer already connected: #{pubkey}")
          {:ok, %Lightning.ConnectPeerResponse{}}
        else
          Logger.error("connect_peer failed: #{msg}")
          error
        end

      {:error, error} ->
        Logger.error("connect_peer failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  @spec disconnect_peer(Conn.t(), String.t()) ::
          {:ok, Lightning.DisconnectPeerResponse.t()} | {:error, term()}
  def disconnect_peer(%Conn{} = conn, pubkey) when is_binary(pubkey) do
    ...
    case Lightning.Stub.disconnect_peer(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        Logger.info("Disconnected from peer: #{pubkey}")
        {:ok, response}

      {:error, error} ->
        Logger.error("disconnect_peer failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  @spec list_peers(Conn.t(), keyword()) ::
          {:ok, Lightning.ListPeersResponse.t()} | {:error, term()}
  def list_peers(%Conn{} = conn, opts \\ []) do
    ...
    case Lightning.Stub.list_peers(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        Logger.debug("Listed #{length(response.peers)} peers")
        {:ok, response}

      {:error, error} ->
        Logger.error("list_peers failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  **Important**: The peer must be connected first via `connect_peer/4`.
  ...
  @spec open_channel_sync(Conn.t(), String.t(), integer(), keyword()) ::
          {:ok, Lightning.ChannelPoint.t()} | {:error, term()}
  def open_channel_sync(conn, node_pubkey, local_funding_amount, opts \\ [])
  ...
  def open_channel_sync(%Conn{} = conn, node_pubkey, local_funding_amount, opts)
    ...
    Logger.info(
      "Opening channel to #{node_pubkey} for #{local_funding_amount} sats" <>
        if(opts[:private], do: " (private)", else: "")
    )

    case Lightning.Stub.open_channel_sync(conn.channel, request, metadata: metadata) do
      {:ok, channel_point} ->
        # Extract txid from oneof field
        # txid_hex =
        #   case channel_point.funding_txid do
        #     {:funding_txid_bytes, bytes} -> Base.encode16(bytes, case: :lower)
        #     {:funding_txid_str, str} -> str
        #     _ -> "unknown"
        #   end

        # Logger.info("Channel opened: #{txid_hex}:#{channel_point.output_index}")
        {:ok, channel_point}

      {:error, error} ->
        Logger.error("open_channel_sync failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  @spec list_channels(Conn.t(), keyword()) ::
          {:ok, Lightning.ListChannelsResponse.t()} | {:error, term()}
  def list_channels(%Conn{} = conn, opts \\ []) do
    ...
    case Lightning.Stub.list_channels(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        Logger.debug("Listed #{length(response.channels)} channels")
        {:ok, response}

      {:error, error} ->
        Logger.error("list_channels failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  * Waiting close - Waiting for close transaction confirmation.
  ...
  @spec pending_channels(Conn.t()) ::
          {:ok, Lightning.PendingChannelsResponse.t()} | {:error, term()}
  def pending_channels(%Conn{} = conn) do
    request = %Lightning.PendingChannelsRequest{}
    metadata = Conn.grpc_metadata(conn)

    case Lightning.Stub.pending_channels(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        {:ok, response}

      {:error, error} ->
        Logger.error("pending_channels failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  @spec closed_channels(Conn.t(), keyword()) ::
          {:ok, Lightning.ClosedChannelsResponse.t()} | {:error, term()}
  def closed_channels(%Conn{} = conn, opts \\ []) do
    ...
    case Lightning.Stub.closed_channels(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        {:ok, response}

      {:error, error} ->
        Logger.error("closed_channels failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  @spec new_address(Conn.t(), keyword()) ::
          {:ok, Lightning.NewAddressResponse.t()} | {:error, term()}
  def new_address(%Conn{} = conn, opts \\ []) do
    ...
    case Lightning.Stub.new_address(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        Logger.debug("Generated new address: #{response.address}")
        {:ok, response}

      {:error, error} ->
        Logger.error("new_address failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  @spec wallet_balance(Conn.t()) ::
          {:ok, Lightning.WalletBalanceResponse.t()} | {:error, term()}
  def wallet_balance(%Conn{} = conn) do
    request = %Lightning.WalletBalanceRequest{}
    metadata = Conn.grpc_metadata(conn)

    case Lightning.Stub.wallet_balance(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        {:ok, response}

      {:error, error} ->
        Logger.error("wallet_balance failed: #{inspect(error)}")
        {:error, error}
    end
  end
  ...
  @spec channel_balance(Conn.t()) ::
          {:ok, Lightning.ChannelBalanceResponse.t()} | {:error, term()}
  def channel_balance(%Conn{} = conn) do
    request = %Lightning.ChannelBalanceRequest{}
    metadata = Conn.grpc_metadata(conn)

    case Lightning.Stub.channel_balance(conn.channel, request, metadata: metadata) do
      {:ok, response} ->
        {:ok, response}

      {:error, error} ->
        Logger.error("channel_balance failed: #{inspect(error)}")
        {:error, error}
    end
  end
